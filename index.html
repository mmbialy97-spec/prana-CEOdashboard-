...
let state = {
  ...
  tab: 'overview',      // overview | members | revenue | classes
  fileMap: {},
  processing: false,
  dragging: false,
  dashData: null,
  weeks: [],
  selectedWeek: null,
  allWeeksData: [],     // full data for all weeks (loaded when Trends tab opened)
...
function handleDorianDownload() {
  const dorian = state.dashData?.current?.dorian || {};
  const all = [
    ...(dorian.critical||[]).map(m => ({...m, priority:'Critical'})),
    ...(dorian.watch||[]).map(m => ({...m, priority:'Watch'})),
    ...(dorian.lost||[]).map(m => ({...m, priority:'Lost'})),
    ...(dorian.win_back||[]).map(m => ({...m, priority:'Win-Back'})),
  ];
  if (!all.length) return alert('No members in action list.');
  downloadCSV(all, 'dorian-action-list-' + (state.dashData?.current?.week_of||'') + '.csv');
}
...
function aggregateData() {
  ...
  // Fix Mindbody "Last, First" name format → "First Last"
  ...
  // Derive week_of from sales CSV dates
  // Simple approach: find the most recent date, get its Monday
  ...
  function mondayOfDate(y, m, d) {
    // Pure arithmetic day-of-week (Sakamoto's algorithm) — no Date object, no timezone issues
    ...
    // Subtract daysBack from date without using Date object
  ...
  try {
    ...
    // If sales CSV empty or inconclusive, try attendance CSV
  ...
  // Fallback: Monday of current local week
  if (!weekOf) {
    const now = new Date();
  ...
  // Week start = Monday midnight — parse weekOf string manually to avoid timezone issues
  ...
  // 12_class_visit is all-time data — we use 09_attendance_analysis for this week's class data instead

  // ── Revenue ──
  // Sales total = sum of Item Total from sales report
  const sales_total = salesRows.reduce((s,r) => {
    return s + toNum(r['Item Total'] || r['Total'] || r['Amount'] || 0);
  }, 0);

  // MRR = active Founder Members × $199 (always, regardless of promotional pricing)
  // This is the source of truth — autopay CSV has partial cycles and promo rates
  const autopay_total = 0; // calculated after active_count is known

  // ── Membership ──
  const active_count    = activeRows.length;
  const mrr             = active_count * 199; // $199/month per Founder Member — always
  const cancelled_count = cancelRows.length;

  // New FOUNDER MEMBERS this week = retention_management rows where:
  // 1. Membership Type contains "Founder"
  // 2. Member Since date is within past 7 days
  ...
  // First time visitors this week (ClassPass, drop-ins — informational only)
  const first_time_visitors = firstRows.length;

  // ── Build Founder Member health map from retention_management ──
  const founderMap = {};
  retMgmtRows.forEach(r => {
    ...
    let daysAbsent = null;
    try { const d = new Date(lv); if (!isNaN(d)) daysAbsent = Math.floor((new Date()-d)/86400000); } catch {}
    let health = 'green';
  ...
  // ── Build Dorian list entirely browser-side from founderMap ──
  // This ensures ALL at-risk members appear regardless of no_return CSV
  ...
  // Members with 0 visits (joined but never came) - also urgent
  ...
  // ── No-return Founder Members — enriched ──
  ...
  // ── New Founder Members this week — need first visit follow-up ──
  ...
  // ── Cancelled members with full visit history ──
  const cancelled_members = cancelRows
    ...
    .map(r => {
      ...
      return {
        ...
        cancel_date: (r['Cancellation Date']||'').replace('Terminated from ','').trim(),
  ...
  attendRows.forEach(r => {
    ...
    // Fix staff name "Last, First" → "First Last"
...
async function processFiles() {
  ...
  try {
    // 1. Aggregate all browser-side data (class breakdown, health, instructors etc.)
    const browserData = aggregateData();

    // 2. Send to Vercel — Claude analyses, saves to Sheets, returns result
    ...
    // 3. Merge browser-calculated fields into the Claude result
    ...
    // Override Claude's dorian list with browser-calculated one — contains ALL at-risk members
    current.dorian              = browserData.browser_dorian;

    // 4. Save enriched result back to Sheets via /api/save
    ...
    // 5. Update cache — add new week, keep existing weeks
    // Remove this week if it already exists (re-upload)
    state.allWeeksData = state.allWeeksData.filter(w => w.week_of !== current.week_of);
    // Add new week in chronological order
    state.allWeeksData.push(current);
    state.allWeeksData.sort((a,b) => a.week_of > b.week_of ? 1 : -1);
    // Update weeks list and selected week
...
function renderTrends() {
  ...
  // Aggregations
  const totalNewMembers   = sorted.reduce((s,w) => s + (w.membership?.new_this_week||0), 0);
  const totalChurned      = sorted.reduce((s,w) => s + (w.membership?.churned_this_week||0), 0);
  const totalNetGrowth    = sorted.reduce((s,w) => s + (w.membership?.net_growth||0), 0);
  const totalVisits       = sorted.reduce((s,w) => s + (w.attendance?.total_visits||0), 0);
  const avgChurnRate      = sorted.reduce((s,w) => s + (w.membership?.churn_rate_pct||0), 0) / n;
  const avgVisitsPerMember= sorted.reduce((s,w) => s + (w.avg_founder_visits||0), 0) / n;
  const avgNewPerWeek     = Math.round(totalNewMembers / n * 10) / 10;
  const avgChurnPerWeek   = Math.round(totalChurned / n * 10) / 10;
  const firstMRR          = first.revenue?.mrr || 0;
  const latestMRR         = latest.revenue?.mrr || 0;
  const mrrGrowth         = latestMRR - firstMRR;
  const firstMembers      = first.membership?.active_count || 0;
  const latestMembers     = latest.membership?.active_count || 0;
  const memberGrowth      = latestMembers - firstMembers;

  // Best and worst weeks
  const bestMRRWeek     = sorted.reduce((best,w) => (w.revenue?.mrr||0) > (best.revenue?.mrr||0) ? w : best, sorted[0]);
  const worstChurnWeek  = sorted.reduce((worst,w) => (w.membership?.churned_this_week||0) > (worst.membership?.churned_this_week||0) ? w : worst, sorted[0]);
  const bestGrowthWeek  = sorted.reduce((best,w) => (w.membership?.new_this_week||0) > (best.membership?.new_this_week||0) ? w : best, sorted[0]);
  ...
  const wowArrow = (val) => val > 0 ? '<span style="color:var(--green)">↑</span>' : val < 0 ? '<span style="color:var(--red)">↓</span>' : '';

  return `
    <div style="font-size:11px;font-weight:600;letter-spacing:0.12em;color:var(--ink-3);text-transform:uppercase;margin-bottom:16px">${n} weeks of data · ${first.week_of} → ${latest.week_of}</div>

    <!-- Member growth -->
    <div style="font-size:12px;font-weight:600;color:var(--ink-2);margin-bottom:10px;text-transform:uppercase;letter-spacing:0.08em">Membership</div>
    <div class="g3" style="margin-bottom:20px">
      ${trendCard('Members Now', latestMembers, 'Started at '+firstMembers+' · '+wowArrow(memberGrowth)+' '+Math.abs(memberGrowth)+' total growth', memberGrowth >= 0 ? 'var(--green)' : 'var(--red)')}
      ${trendCard('Total New Members', totalNewMembers, avgNewPerWeek+' avg per week', 'var(--green)')}
      ${trendCard('Total Churned', totalChurned, avgChurnPerWeek+' avg per week · '+Math.round(avgChurnRate*10)/10+'% avg churn rate', totalChurned > 0 ? 'var(--red)' : 'var(--ink)')}
    </div>

    <!-- Revenue -->
    <div style="font-size:12px;font-weight:600;color:var(--ink-2);margin-bottom:10px;text-transform:uppercase;letter-spacing:0.08em">Revenue</div>
    <div class="g3" style="margin-bottom:20px">
      ${trendCard('MRR Now', fmtC(latestMRR), 'Started at '+fmtC(firstMRR)+' · '+wowArrow(mrrGrowth)+fmtC(Math.abs(mrrGrowth))+' growth', 'var(--green)')}
      ${trendCard('Forecasted ARR', fmtC(latestMRR*12), 'Based on current MRR', 'var(--green)')}
      ${trendCard('Revenue / Member', fmtC(latest.revenue?.revenue_per_member||0), 'Latest week · started at '+fmtC(first.revenue?.revenue_per_member||0), 'var(--ink)')}
    </div>

    <!-- Attendance -->
    <div style="font-size:12px;font-weight:600;color:var(--ink-2);margin-bottom:10px;text-transform:uppercase;letter-spacing:0.08em">Attendance</div>
    <div class="g3" style="margin-bottom:20px">
      ${trendCard('Total Check-ins', totalVisits.toLocaleString(), 'Across all '+n+' weeks', 'var(--ink)')}
      ${trendCard('Avg Visits / Member', Math.round(avgVisitsPerMember*10)/10+'/wk', 'Average across all weeks · target 3+', (avgVisitsPerMember>=3?'var(--green)':avgVisitsPerMember>=2?'var(--amber)':'var(--red)'))}
      ${trendCard('Net Growth Total', (totalNetGrowth>=0?'+':'')+totalNetGrowth, 'Members gained minus lost over '+n+' weeks', totalNetGrowth>=0?'var(--green)':'var(--red)')}
    </div>

    <!-- Notable weeks -->
    <div style="font-size:12px;font-weight:600;color:var(--ink-2);margin-bottom:10px;text-transform:uppercase;letter-spacing:0.08em">Notable Weeks</div>
    <div class="g3" style="margin-bottom:20px">
      ${trendCard('Best MRR Week', fmtC(bestMRRWeek.revenue?.mrr||0), bestMRRWeek.week_of, 'var(--green)')}
      ${trendCard('Best Growth Week', '+'+(bestGrowthWeek.membership?.new_this_week||0)+' members', bestGrowthWeek.week_of, 'var(--green)')}
      ${trendCard('Highest Churn Week', (worstChurnWeek.membership?.churned_this_week||0)+' churned', worstChurnWeek.week_of, 'var(--red)')}
    </div>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead>
            <tr style="border-bottom:2px solid var(--border)">
              ...
              <th style="text-align:right;padding:8px 10px;font-size:10px;font-weight:600;color:var(--ink-3);text-transform:uppercase;white-space:nowrap">Visits</th>
          ...
          <tbody>
            ${sorted.map((w,i) => {
              ...
              return '<tr style="border-bottom:1px solid var(--border);background:' + (isLatest ? 'var(--green-light)' : 'transparent') + '">' +
                ...
                '<td style="padding:9px 10px;text-align:right;font-family:var(--mono);font-size:12px;color:var(--ink)">' + (w.attendance?.total_visits||0).toLocaleString() + '</td>' +
...
// ─── Load all weeks — already in memory from init ────────────────
function loadAllWeeks() {
  // Data is already loaded in state.allWeeksData during init
  // Just re-render to show Trends
  state.allWeeksLoading = false;
  render();
}

// ─── Switch week — uses cache ─────────────────────────────────────
...
// ─── Init — load ALL weeks into memory on open ───────────────────
async function init() {
  state.view = 'loading';
  render();

  try {
    // Step 1: Always load latest first — guaranteed to work
    ...
    // Build initial cache from latest + previous
    const allData0 = [];
    ...
    state.view         = 'dashboard';
    render(); // show dashboard immediately with correct latest week

    // Step 2: load ALL weeks in background for Trends + week switcher
    try {
      const weeksData = await fetch('/api/read?action=read_weeks').then(r => r.json());
      const weeks = (weeksData.weeks || []).sort((a,b) => a.week_of > b.week_of ? 1 : -1); // oldest first
      const allData = [];
      ...
      if (allData.length) {
        ...
        // Always show the most recent week
    ...
    } catch {} // background load failure is non-fatal
